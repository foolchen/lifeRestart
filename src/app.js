import { getGrade, getRate } from "./functions/addition.js";
...
class App {
  ...
  initPages() {
    ...
    indexPage.find("#save").click(() => {
      ...
      a.download = `Remake_save_${new Date()
        .toISOString()
        .replace(":", ".")}.json`;
    ...
    indexPage.find("#load").click(() => {
      const file = $(
        `<input type="file" name="file" accept="application/json" style="display: none;" />`
      );
      file.appendTo("body");
      file.click();
      file.on("change", (e) => {
    ...
    trajectoryPage.find("#auto10x").click(() => auto(100));
